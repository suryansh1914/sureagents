...
gh api repos/suryansh1914/sureagents/issues/<number>/comments --jq '.[].user.login'

# Get PR review comments
gh api repos/suryansh1914/sureagents/pulls/<number>/comments --jq '.[].user.login'
```
...
8. **Full Changelog link**:
   ```
   **Full Changelog**: https://github.com/suryansh1914/sureagents/compare/<prev-tag>...<new-tag>
   ```
...
- Add an import path fix (Pi uses `../generated/` not `@sureagents/shared` or `@sureagents/ai`)
...
3. **The pipeline handles everything else:**
   ...
   - Publishes `@sureagents/opencode` and `@sureagents/pi-extension` to npm with provenance
...
4. **Monitor the pipeline:**
   ...
   - npm packages published successfully (check with `npm view @sureagents/opencode version` and `npm view @sureagents/pi-extension version`)
