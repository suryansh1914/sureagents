...
const CATEGORY = "SureAgents";
const INSTALL_URL = "https://sureagents.ai/docs/getting-started/installation/";
...
let runtimePromise: Promise<SureAgentsRuntime> | null = null;

export default function sureagentsAmpPlugin(amp: PluginAPI) {
  amp.logger.log("[sureagents] Amp plugin initialized");
  ...
  amp.registerCommand(
    "sureagents-review",
    {
      ...
      category: CATEGORY,
      description: "Open SureAgents code review for the current workspace changes.",
    },
    async (ctx) => {
      const result = await runSureAgents(amp, ctx, ["review"]);
      await handleReviewResult(ctx, result);
    },
  );

  amp.registerCommand(
    "sureagents-review-target",
    {
      title: "Review changes or PR",
      category: CATEGORY,
      description: "Open SureAgents code review for local changes, a PR/MR URL, or review arguments.",
    },
    async (ctx) => {
      ...
      const result = await runSureAgents(amp, ctx, ["review", ...reviewArgs]);
      await handleReviewResult(ctx, result);
    },
  );
  ...
  amp.registerCommand(
    "sureagents-annotate",
    {
      title: "Annotate file",
      category: CATEGORY,
      description: "Open SureAgents annotation UI for a markdown/html file, folder, or URL.",
    },
    async (ctx) => {
      const target = await ctx.ui.input({
        title: "Annotate",
      ...
      const result = await runSureAgents(amp, ctx, ["annotate", ...args, "--json"]);
  ...
  amp.registerCommand(
    "sureagents-last",
    {
      ...
      category: CATEGORY,
      description: "Open SureAgents annotation UI for Amp's latest assistant message.",
    },
    async (ctx) => {
      if (!ctx.thread) {
      ...
      const runtime = await getSureAgentsRuntime();
      let tempFile: string | null = null;
      let result: RunResult;

      try {
        if (runtime.features.stdinLast) {
          result = await runSureAgents(
            amp,
            ctx,
          ...
          tempFile = join(tmpdir(), `sureagents-amp-last-${process.pid}-${Date.now()}-${randomUUID()}.md`);
          writeFileSync(tempFile, message, "utf8");
          result = await runSureAgents(amp, ctx, ["annotate", tempFile, "--json"], { runtime });
        }
      } finally {
        if (tempFile) {
          try {
...
export function formatAnnotationFeedback(
  ...
  const config = loadSureAgentsConfig();
...
async function appendFeedback(ctx: CommandContext, content: string): Promise<void> {
  if (!ctx.thread) {
    await ctx.ui.notify("SureAgents produced feedback, but there is no active Amp thread.");
    return;
  }
...
async function notifyFailure(
  ...
  await ctx.ui.notify(`SureAgents ${mode} failed.${details ? `\n\n${details}` : ""}${installHint}`);
  return true;
}

async function runSureAgents(
  amp: PluginAPI,
  ctx: CommandContext,
  args: string[],
  options: { stdin?: string; runtime?: SureAgentsRuntime } = {},
): Promise<RunResult> {
  const cwd = await resolveCwd(ctx);
  const runtime = options.runtime ?? await getSureAgentsRuntime();
  const readyFile = runtime.features.readyFile
    ? join(tmpdir(), `sureagents-amp-${process.pid}-${Date.now()}-${randomUUID()}.jsonl`)
    : null;
  const command = [...runtime.command, ...args];
  const env = buildEnv(buildSureAgentsEnv(cwd, readyFile));
  ...
  return {
    ...
    stdout,
    stderr,
    ...(readyTimedOut ? { error: "Timed out waiting for SureAgents to publish its browser URL." } : {}),
  };
}

export async function resolveCwd(ctx: CommandContext): Promise<string> {
  const explicitCwd = normalizeDirectory(process.env.SUREAGENTS_CWD);
...
export function buildSureAgentsEnv(cwd: string, readyFile: string | null): Record<string, string> {
  return {
    SUREAGENTS_ORIGIN: RUNTIME,
    SUREAGENTS_CWD: cwd,
    ...(readyFile ? { SUREAGENTS_READY_FILE: readyFile } : {}),
  };
}
...
async function notifyUrls(
  ...
  for (const rawUrl of matches) {
    ...
    seenUrls.add(url);
    await ctx.ui.notify(`SureAgents link:\n${url}`);
  }
}
...
async function getSureAgentsRuntime(): Promise<SureAgentsRuntime> {
  runtimePromise ??= resolveSureAgentsRuntime();
  return runtimePromise;
}

async function resolveSureAgentsRuntime(): Promise<SureAgentsRuntime> {
  const explicitSource = process.env.SUREAGENTS_AMP_SOURCE_ENTRY;
  const sourceEntry = explicitSource
    ? resolve(explicitSource)
    : process.env.SUREAGENTS_AMP_USE_SOURCE === "1"
  ...
  const { command, version } = resolveSureAgentsCommand();
  return {
    command,
    source: "cli",
    version,
...
function resolveSureAgentsCommand(): { command: string[]; version: string | null } {
  const candidates = getSureAgentsCommandCandidates();
  let fallback = candidates[candidates.length - 1] ?? ["sureagents"];
  ...
  for (const command of candidates) {
    ...
    if (isPathLike(executable)) {
      ...
      const version = detectSureAgentsVersion(command);
  ...
  return { command: fallback, version: detectSureAgentsVersion(fallback) };
}

export function getSureAgentsCommandCandidates(
  options: {
  ...
  const explicitBin = normalizeExecutablePath(env.SUREAGENTS_BIN);
  if (explicitBin) candidates.push([explicitBin]);

  if (platform === "win32") {
    const localAppData = normalizeExecutablePath(env.LOCALAPPDATA);
    if (localAppData) candidates.push([join(localAppData, "sureagents", "sureagents.exe")]);

    for (const home of homes) {
      candidates.push([join(home, ".local", "bin", "sureagents.exe")]);
    }
  } else {
    for (const home of homes) {
      candidates.push([join(home, ".local", "bin", "sureagents")]);
    }
  }

  candidates.push(["sureagents"]);
  ...
  while (true) {
    ...
    if (existsSync(packageJsonPath)) {
      try {
        ...
        if (pkg.name === "sureagents") return dir;
      } catch {
...
function detectSureAgentsVersion(command: string[]): string | null {
  try {
...
function loadSureAgentsConfig(): PromptConfig {
  try {
    const configPath = join(getSureAgentsDataDir(), "config.json");
...
export function getSureAgentsDataDir(): string {
  const value = process.env.SUREAGENTS_DATA_DIR?.trim();
  if (!value) return join(homedir(), ".sureagents");
