...
- **Policy file** at `~/.gemini/policies/sureagents.toml` — allows `exit_plan_mode` without the TUI confirmation dialog
- **Hook** in `~/.gemini/settings.json` — intercepts `exit_plan_mode` and opens the browser review UI
- **Slash commands** at `~/.gemini/commands/` — `/sureagents-review` and `/sureagents-annotate`
...
3. The `BeforeTool` hook intercepts the call, reads the plan from disk, and opens the SureAgents review UI in your browser
...
| `/sureagents-review <pr-url>` | Review a GitHub pull request |
...
|----------|-------------|
| `SUREAGENTS_REMOTE` | Set to `1` for remote mode (devcontainer, SSH). Uses fixed port and skips browser open. |
