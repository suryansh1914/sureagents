---
title: "SureAgents Meets Pi"
description: "SureAgents now supports Pi, the minimal terminal coding agent. File-based plan mode, bash safety gating, progress tracking — all with the same browser review UI."
date: 2026-02-21
author: "backnotprop"
tags: ["pi", "integration", "plan-mode"]
---

**SureAgents now supports [Pi](https://github.com/earendil-works/pi), the minimal terminal coding agent originally created by Mario Zechner (now part of Earendil Works).** Install it as a Pi extension and you get file-based plan mode, visual plan review, code review, and markdown annotation — all through the same browser UI that Claude Code and OpenCode users already know.

## Watch the Demo

<iframe width="100%" style="aspect-ratio: 16/9;" src="https://www.youtube.com/embed/XqFun9XCXPw?si=BbjywvxWPONkLRij" title="SureAgents for Pi" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
...
This is a good fit for SureAgents. Instead of being a hook that intercepts a built-in plan mode (like the Claude Code integration), the Pi extension *is* the plan mode. It registers the tools, manages the state machine, gates the permissions, and handles the browser UI — all as a single extension.
...
Start plan mode with `pi --plan` or toggle it mid-session with `/sureagents` or `Ctrl+Alt+P`. The extension immediately restricts the agent's available tools:
...
- **`sureagents_submit_plan` tool available** — this is how the agent signals the plan is ready for review.

The agent explores your codebase using read-only tools, drafts a plan as a markdown checklist in `PLAN.md`, and calls `sureagents_submit_plan` when ready. The planning prompt instructs the agent to work iteratively — explore, update the plan, ask questions, repeat — rather than dumping a fully-formed plan in one shot.
...
When the agent calls `sureagents_submit_plan`, the extension reads `PLAN.md` from disk, starts a local HTTP server, and opens the SureAgents browser UI. You see the rendered plan with all the annotation tools — comments, deletions, global comments, quick labels, and "looks good" approvals.
...
- **Deny with annotations** — the extension stays in planning. The agent receives your structured feedback and is told to use `edit` (not `write`) to make targeted revisions, then call `sureagents_submit_plan` again.
...
Pi loads extensions via [jiti](https://github.com/unjs/jiti), which runs in Node.js — not Bun. The shared SureAgents server code in `packages/server/` uses `Bun.serve()`, which doesn't work here.
...
The Pi extension includes the same `/sureagents-review` and `/sureagents-annotate` commands as the Claude Code and OpenCode integrations:

- **`/sureagents-review`** — captures `git diff HEAD`, opens the diff viewer in your browser. You annotate lines, switch between diff types (uncommitted, last commit, vs default branch), and submit feedback. The extension sends the formatted feedback back to the agent as a user message via `pi.sendUserMessage()`.

- **`/sureagents-annotate <file.md>`** — opens any markdown file in the annotation UI. Useful for reviewing design docs or specs with the agent. Feedback goes back as a user message referencing the file path.
...
SureAgents now supports three coding agents — Claude Code, OpenCode, and Pi — each with a different integration model:
...
| **Review trigger** | `ExitPlanMode` hook event | `submit_plan` tool call | `sureagents_submit_plan` tool call |
