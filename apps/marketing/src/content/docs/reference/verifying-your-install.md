...
description: "SHA256 checksums and SLSA build provenance verification for SureAgents binaries."
sidebar:
  order: 4
section: "Reference"
---
...
```bash
gh attestation verify ~/.local/bin/sureagents \
...
```powershell
gh attestation verify "$env:LOCALAPPDATA\sureagents\sureagents.exe" `
  --repo suryansh1914/sureagents `
  --source-ref refs/tags/vX.Y.Z `
  --signer-workflow suryansh1914/sureagents/.github/workflows/release.yml
```
...
```cmd
gh attestation verify "%USERPROFILE%\.local\bin\sureagents.exe" ^
  --repo suryansh1914/sureagents ^
  --source-ref refs/tags/vX.Y.Z ^
  --signer-workflow suryansh1914/sureagents/.github/workflows/release.yml
```
...
```bash
export SUREAGENTS_VERIFY_ATTESTATION=1
```
...
```bash
mkdir -p ~/.sureagents
echo '{ "verifyAttestation": true }' > ~/.sureagents/config.json
```
