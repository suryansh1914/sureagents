...
SureAgents integration for the [Pi coding agent](https://github.com/earendil-works/pi). Adds file-based plan mode with a visual browser UI for reviewing, annotating, and approving agent plans.
...
pi install ./sureagents/apps/pi-extension
```
...
Or toggle it during a session with `/sureagents` or `Ctrl+Alt+P`. The command accepts an optional file path argument (`/sureagents plans/auth.md`) or prompts you to choose one interactively.
...
When the agent calls `sureagents_submit_plan`, the SureAgents UI opens in your browser. You can:
...
Other Pi extensions can enter, exit, toggle, or query SureAgents plan mode through the shared Pi event bus without invoking the `/sureagents` slash command:

```ts
import { SUREAGENTS_REQUEST_CHANNEL } from "@sureagents/pi-extension/sureagents-events";

const response = await new Promise((resolve) => {
  pi.events.emit(SUREAGENTS_REQUEST_CHANNEL, {
...
SureAgents loads configuration in three layers:

1. Built-in base config shipped with the package: `sureagents.json`
2. Global user config: `~/.pi/agent/sureagents.json`
3. Project-local config: `<cwd>/.pi/sureagents.json`
...
```json
{
  ...
  "phases": {
    "planning": {
      "model": null,
      "thinking": null,
      "activeTools": ["grep", "find", "ls", "sureagents_submit_plan"],
      "statusLabel": "⏸ plan",
...
- `activeTools` are additive with the tools currently active in the session, so SureAgents still preserves tools provided by other extensions.
- Execution progress remains dynamic (`[DONE:n]` + checklist tracking), even if `statusLabel` is set.

#### Example files

- Built-in base config shipped with the package: `apps/pi-extension/sureagents.json`
- Global user override: `~/.pi/agent/sureagents.json`
- Project-local override: `<cwd>/.pi/sureagents.json`

### Code review

Run `/sureagents-review` to open your current git changes in the code review UI. Annotate specific lines, switch between diff views (uncommitted, staged, last commit, branch), and submit feedback that gets sent to the agent.

### Shared SureAgents event API

SureAgents also listens on the shared `sureagents:request` event channel so other extensions can reuse the same browser review flows without importing SureAgents internals.
...
- callers send `sureagents:request` with action `plan-review`
- SureAgents opens the browser review and immediately responds with `{ status: "handled", result: { status: "pending", reviewId } }`
- when the human approves or rejects in the browser, SureAgents emits `sureagents:review-result` with `{ reviewId, approved, feedback, savedPath?, agentSwitch?, permissionMode? }`
...
Run `/sureagents-annotate <file.md>` to open any markdown file in the annotation UI. Useful for reviewing documentation or design specs with the agent.

### Annotate last message

Run `/sureagents-last` to annotate the agent's most recent response. The message opens in the annotation UI where you can highlight text, add comments, and send structured feedback back to the agent.

### Archive browser

The SureAgents archive browser is available through the shared event API as `archive`, which opens the saved plan/decision browser for future callers. The orchestrator does not expose a dedicated archive command yet.
...
| Command | Description |
|---------|-------------|
| `/sureagents` | Toggle plan mode. The agent writes a markdown plan file anywhere in the working directory and submits its path |
| `/sureagents-review` | Open code review UI for current changes |
| `/sureagents-annotate <file>` | Open markdown file in annotation UI |
| `/sureagents-last` | Annotate the last assistant message |
