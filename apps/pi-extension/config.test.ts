...
import { loadSureAgentsConfig, formatTodoList, renderTemplate, resolvePhaseProfile } from "./config";
...
describe("sureagents config", () => {
  test("loads the shipped internal base config", () => {
    const cwdDir = makeTempDir("sureagents-config-base-");
    process.env.HOME = makeTempDir("sureagents-config-home-base-");

    const loaded = loadSureAgentsConfig(cwdDir);
    const planning = resolvePhaseProfile(loaded.config, "planning");

    expect(loaded.warnings).toEqual([]);
    expect(planning.statusLabel).toBe("⏸ plan");
    expect(planning.activeTools).toEqual(["grep", "find", "ls", "sureagents_submit_plan"]);
  });

  test("allows a project config to clear an inherited phase with null", () => {
    const homeDir = makeTempDir("sureagents-config-home-null-");
    const cwdDir = makeTempDir("sureagents-config-cwd-null-");
    ...
    writeFileSync(
      join(globalConfigDir, "sureagents.json"),
      JSON.stringify({
    ...
    writeFileSync(
      join(projectConfigDir, "sureagents.json"),
      JSON.stringify({
  ...
  test("loads global and project configs with project precedence", () => {
    const homeDir = makeTempDir("sureagents-config-home-");
    const cwdDir = makeTempDir("sureagents-config-cwd-");
    process.env.HOME = homeDir;
