...
type SureAgentsGlobal = typeof globalThis & {
	__sureagentsCurrentPiSession?: CurrentPiSessionStore;
};
...
const globalStore = globalThis as SureAgentsGlobal;

function getStore(): CurrentPiSessionStore {
	globalStore.__sureagentsCurrentPiSession ??= {};
	return globalStore.__sureagentsCurrentPiSession;
}
...
export function registerCurrentPiSession(pi: ExtensionAPI): CurrentPiSessionRegistration {
	const token = Symbol("sureagents-current-pi-session");
...
export function notifyCurrentPiSession(
	...
	try {
		...
		console.error(`SureAgents current-session notification failed: ${getErrorMessage(err)}`);
		return false;
	}
}
...
export function withCurrentPiSessionFallbackHeader(content: SendUserMessageContent): SendUserMessageContent {
	...
	return `This SureAgents feedback was submitted from a browser tab opened before Pi switched sessions. It is being delivered to ${getCurrentPiSessionLabel()} because the original Pi session is no longer active.
