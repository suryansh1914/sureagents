/**
 * SureAgents Pi Extension — File-based plan mode with visual browser review.
 *
 * During planning the agent writes any markdown file anywhere inside cwd and
 * calls sureagents_submit_plan with the path. The user reviews in the
 * browser UI and can approve, deny with annotations, or request changes.
 *
 * Features:
 * - /sureagents command or Ctrl+Alt+P to toggle
 ...
 * - sureagents_submit_plan tool with browser-based visual approval
 * - [DONE:n] markers for execution progress tracking
 * - /sureagents-review command for code review
 * - /sureagents-annotate command for markdown annotation
 */
...
import { buildPromptVariables, formatTodoList, loadSureAgentsConfig, renderTemplate, resolvePhaseProfile } from "./config.js";
import {
	type ChecklistItem,
	markCompletedSteps,
	parseChecklist,
...
import {
	...
	registerSureAgentsEventListeners,
} from "./sureagents-events.js";
import {
...
function getPlanReviewAvailabilityWarning(options: { hasUI: boolean; hasPlanHtml: boolean }): string | null {
	...
	if (!hasUI && !hasPlanHtml) {
		return "SureAgents: interactive plan review is unavailable in this session (no UI support and missing built assets). Plans will auto-approve on exit_plan_mode.";
	}
	if (!hasUI) {
		return "SureAgents: interactive plan review is unavailable in this session (no UI support). Plans will auto-approve on exit_plan_mode.";
	}
	return "SureAgents: interactive plan review assets are missing. Rebuild the extension to restore the browser UI. Plans will auto-approve on exit_plan_mode.";
}
...
function safeNotify(
	...
	try {
		...
		console.error(`SureAgents notification failed: ${err instanceof Error ? err.message : String(err)}`);
	}
}
...
export default function sureagents(pi: ExtensionAPI): void {
	const currentPiSession = registerCurrentPiSession(pi);
	let phase: Phase = "idle";
	void registerSureAgentsEventListeners(pi, {
	...
	function getPhaseProfile(): ReturnType<typeof resolvePhaseProfile> | undefined {
		if (phase === "planning" || phase === "executing") {
			return resolvePhaseProfile(sureagentsConfig, phase);
		}
		return undefined;
	}
	...
	function updateStatus(ctx: ExtensionContext): void {
		...
		if (phase === "executing" && checklistItems.length > 0) {
			...
			ctx.ui.setStatus("sureagents", ctx.ui.theme.fg("warning", profile.statusLabel));
		} else if (phase === "executing" && profile?.statusLabel) {
			ctx.ui.setStatus("sureagents", ctx.ui.theme.fg("accent", profile.statusLabel));
		} else {
			ctx.ui.setStatus("sureagents", undefined);
		}
	}
	...
	function updateWidget(ctx: ExtensionContext): void {
		if (phase === "executing" && checklistItems.length > 0) {
			...
			ctx.ui.setWidget("sureagents-progress", lines);
		} else {
			ctx.ui.setWidget("sureagents-progress", undefined);
		}
	}
	...
	function persistState(): void {
		...
		pi.appendEntry("sureagents", { phase, lastSubmittedPath, savedState });
	}
	...
	async function applyModelRef(
		...
		if (!model) {
			ctx.ui.notify(`SureAgents: ${reason} model ${ref.provider}/${ref.id} not found.`, "warning");
			return;
		}

		const success = await pi.setModel(model);
		if (!success) {
			ctx.ui.notify(`SureAgents: no API key for ${ref.provider}/${ref.id}.`, "warning");
		}
	}
	...
	async function enterPlanning(ctx: ExtensionContext): Promise<void> {
		...
		persistState();
		ctx.ui.notify(
			"SureAgents: planning mode enabled.",
		);
	...
	async function exitToIdle(ctx: ExtensionContext): Promise<void> {
		...
		updateWidget(ctx);
		persistState();
		ctx.ui.notify("SureAgents: disabled. Full access restored.");
	}
	...
	pi.registerCommand("sureagents", {
		description: "Toggle sureagents planning mode",
		handler: async (_args, ctx) => {
			await togglePlanMode(ctx);
		},
	});

	pi.registerCommand("sureagents-review", {
						...
						try {
							...
							if (result.approved) {
								sendUserMessageWithCurrentSessionFallback(
									...
									{ deliverAs: "followUp" },
									"SureAgents code review feedback could not be sent",
									origin,
								);
								return;
							}
							...
							reportBackgroundError(ctx, "SureAgents code review feedback could not be sent", err, origin);
						}
					})
					.catch((err) => {
						reportBackgroundError(ctx, "SureAgents code review session failed", err, origin);
					});
			} catch (err) {
				ctx.ui.notify(
	...
	pi.registerCommand("sureagents-annotate", {
		...
		handler: async (args, ctx) => {
			...
			if (!filePath) {
				ctx.ui.notify("Usage: /sureagents-annotate <file.md | file.html | https://... | folder/> [--no-jina] [--gate] [--json]", "error");
				return;
			}
					...
					.then((result) => {
						try {
							...
							sendUserMessageWithCurrentSessionFallback(
								...
								{ deliverAs: "followUp" },
								"SureAgents annotation feedback could not be sent",
								origin,
							);
						} catch (err) {
							reportBackgroundError(ctx, "SureAgents annotation feedback could not be sent", err, origin);
						}
					})
					.catch((err) => {
						reportBackgroundError(ctx, "SureAgents annotation session failed", err, origin);
					});
			} catch (err) {
				ctx.ui.notify(
	...
	pi.registerCommand("sureagents-last", {
		description: "Annotate the last assistant message",
		handler: async (args, ctx) => {
			// Support --gate on /sureagents-last for the Stop-hook review gate.
					...
					.then((result) => {
						try {
							...
							sendUserMessageWithCurrentSessionFallback(
								...
								{ deliverAs: "followUp" },
								"SureAgents message annotation feedback could not be sent",
								origin,
							);
						} catch (err) {
							reportBackgroundError(ctx, "SureAgents message annotation feedback could not be sent", err, origin);
						}
					})
					.catch((err) => {
						reportBackgroundError(ctx, "SureAgents message annotation session failed", err, origin);
					});
			} catch (err) {
				ctx.ui.notify(
					`Failed to start annotation UI: ${getStartupErrorMessage(err)}`,
	...
	pi.registerShortcut(Key.ctrlAlt("p"), {
		description: "Toggle sureagents",
		handler: async (ctx) => {
			await togglePlanMode(ctx);
		},
	});

	// ── sureagents_submit_plan Tool ────────────────────────────────────

	pi.registerTool({
		name: PLAN_SUBMIT_TOOL,
		label: "Submit Plan",
		description:
			"Submit your SureAgents plan for user review. " +
			"Call this only while SureAgents planning mode is active, after writing your plan as a markdown file anywhere inside the working directory. " +
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if (phase !== "planning") {
				return {
					content: [
						{
							type: "text",
							text: "Error: Not in plan mode. Use /sureagents to enter planning mode first.",
						},
					],
			...
			if (!ctx.hasUI || !hasPlanBrowserHtml()) {
				...
				pi.appendEntry("sureagents-execute", { lastSubmittedPath });
				persistState();
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (!isPlanWritePathAllowed(inputPath, ctx.cwd)) {
			...
			return {
				block: true,
				reason: `SureAgents: during planning, ${verb} are limited to markdown files (.md, .mdx) inside the working directory. Blocked: ${inputPath}`,
			};
		}
	});
	...
	pi.on("before_agent_start", async (_event, ctx) => {
		...
		if (profile?.systemPrompt) {
			...
			if (rendered.unknownVariables.length > 0) {
				ctx.ui.notify(
					"SureAgents: unknown template variables in " + phase + " prompt: " + rendered.unknownVariables.join(", "),
					"warning",
				);
			}
		...
		if (phase === "planning") {
			return {
				message: {
					customType: "sureagents-context",
					content: `[SUREAGENTS - PLANNING PHASE]
		...
		if (phase === "executing" && checklistItems.length > 0) {
			...
			if (remaining.length > 0) {
				...
				return {
					message: {
						customType: "sureagents-context",
						content: `[SUREAGENTS - EXECUTING PLAN]
...
Execute each step in order. After completing a step, include [DONE:n] in your response where n is the step number.`,
	...
	pi.on("context", async (event) => {
		...
		return {
			messages: event.messages.filter((m) => {
				...
				if (msg.customType === "sureagents-context") return false;
				if (msg.role !== "user") return true;

				const content = msg.content;
				if (typeof content === "string") {
					return !content.includes("[SUREAGENTS -");
				}
				if (Array.isArray(content)) {
					return !content.some(
						(c) =>
							c.type === "text" &&
							(c as { text?: string }).text?.includes("[SUREAGENTS -"),
					);
				}
				return true;
			}),
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		if (checklistItems.every((t) => t.completed)) {
			...
			pi.sendMessage(
				{
					customType: "sureagents-complete",
	...
	pi.on("session_start", async (_event, ctx) => {
		const loadedConfig = loadSureAgentsConfig(ctx.cwd);
		sureagentsConfig = loadedConfig.config;
		for (const warning of loadedConfig.warnings) {
			ctx.ui.notify(`SureAgents config: ${warning}`, "warning");
		}
		...
		const stateEntry = entries
			.filter(
				(e: { type: string; customType?: string }) =>
					e.type === "custom" && e.customType === "sureagents",
			)
			.pop() as { data?: PersistedSureAgentsState } | undefined;
			...
			if (lastSubmittedPath) {
				...
				if (existsSync(fullPath)) {
					...
					for (let i = entries.length - 1; i >= 0; i--) {
						...
						if (entry.customType === "sureagents-execute") {
							executeIndex = i;
							break;
						}
					}
		...
		if (phase === "idle") {
			if (savedState) {
				...
				// Without this, sureagents_submit_plan stays in the active tool set
