...
Read this file only when the user does **not** have a usable SureAgents archive.

This is the secondary path for ordinary Claude Code users whose denial history
exists in `~/.claude/projects/` rather than the SureAgents plans directory (`$SUREAGENTS_DATA_DIR/plans/` or `~/.sureagents/plans/`).
...
In Claude Code fallback mode, report this dataset instead of the SureAgents file counts:
...
Do **not** fabricate SureAgents-only inventory fields in fallback mode:
...
the SureAgents plans directory (`${SUREAGENTS_DATA_DIR:-~/.sureagents}/plans/`) for existing `compound-planning-report*.html` files. If
found, offer the same incremental vs full choice as SureAgents mode. In
...
Use `model: "sonnet"` for reduction agents, same as SureAgents mode. The
...
The SureAgents version uses a revision-rate KPI that may not exist here.
...
denial reasons rather than SureAgents markdown archives.
...
to the SureAgents plans directory (`${SUREAGENTS_DATA_DIR:-~/.sureagents}/plans/`, create the directory if it doesn't exist) so that
