...
function buildSetCookieHeaders(savedCookies: string): string[] {
  ...
  return savedCookies
    .split("; ")
    .filter((c) => c.startsWith("sureagents-"))
...
function injectScript(html: string, savedCookies: string): string {
  ...
  // Virtual cookie jar: overrides document.cookie so sureagents works even
  // when the browser blocks third-party cookies inside the iframe.
  const script = themeListener + `<script>(function(){
      var S=${initial};S["sureagents-auto-close"]="true";
      ...
      try{window.parent.postMessage("sureagents-ready","*");}catch(e){}
    })();</script>`;
