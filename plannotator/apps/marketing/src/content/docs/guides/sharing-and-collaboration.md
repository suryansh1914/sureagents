...
SureAgents lets you share plans and annotations with teammates via URL. Small plans are encoded entirely in the URL hash — no backend, no accounts, no server stores anything. Large plans can optionally use short URLs via the [paste service](/docs/guides/self-hosting/#3-deploy-the-paste-service).
...
```bash
export SUREAGENTS_SHARE=disabled
```
...
When a plan is too large for a URL (~2KB+ compressed), messaging apps like Slack and WhatsApp may truncate it. SureAgents can create a short link by temporarily storing the compressed plan in a paste service.
...
5. A short URL like `share.sureagents.ai/p/aBcDeFgH` is generated
...
By default, share URLs point to `https://share.sureagents.ai`. You can self-host the portal and point SureAgents at your instance. See the [self-hosting guide](/docs/guides/self-hosting/) for details.
