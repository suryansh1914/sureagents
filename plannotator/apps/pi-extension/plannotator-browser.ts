...
try {
	planHtmlContent = readFileSync(resolve(__dirname, "sureagents.html"), "utf-8");
} catch {
...
async function openBrowserForServer(serverUrl: string, ctx: ExtensionContext): Promise<void> {
	...
	if (isRemoteSession()) {
		ctx.ui.notify(`[SureAgents] ${serverUrl}`, "info");
...
function startBrowserDecisionSession<T>(
	...
	const createStoppedError = () => new Error("SureAgents browser session was stopped.");
	const stop = () => {
		if (stopped) return;
		stopped = true;
		server.stop();
...
export async function startPlanReviewBrowserSession(
	...
	if (!ctx.hasUI || !planHtmlContent) {
		throw new Error("SureAgents browser review is unavailable in this session.");
	}
	...
	const server = await startPlanReviewServer({
		...
		origin: "pi",
		sharingEnabled: process.env.SUREAGENTS_SHARE !== "disabled",
		shareBaseUrl: process.env.SUREAGENTS_SHARE_URL || undefined,
		pasteApiUrl: process.env.SUREAGENTS_PASTE_URL || undefined,
	});
...
> {
	if (!ctx.hasUI || !reviewHtmlContent) {
		throw new Error("SureAgents code review browser is unavailable in this session.");
	}
	...
	if (isPRMode && urlArg) {
		...
		if (shouldUseLocalPrCheckout(options)) {
			...
			try {
				...
				sessionDir = join(realpathSync(tmpdir()), `sureagents-pr-${identifier}-${suffix}`);
...
export async function startMarkdownAnnotationSession(
	...
	if (!ctx.hasUI || !planHtmlContent) {
		throw new Error("SureAgents annotation browser is unavailable in this session.");
	}
...
export async function openArchiveBrowserAction(
	...
	if (!ctx.hasUI || !planHtmlContent) {
		throw new Error("SureAgents archive browser is unavailable in this session.");
	}
