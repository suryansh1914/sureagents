---
name: sureagents-compound
disable-model-invocation: true
description: >
  Analyze a user's SureAgents plan archive to extract denial patterns, feedback
  ...
  reasons when SureAgents data is unavailable.
---

# Compound Planning Analysis

You are conducting a comprehensive research analysis of a user's SureAgents plan
...
1. **SureAgents mode (first-class)** — Determine the SureAgents data directory:
   use `$SUREAGENTS_DATA_DIR` if set, otherwise `~/.sureagents`. Check the
   `plans/` subdirectory there. If it exists and contains `*-denied.md` files,
   use this mode. The entire workflow below is written for SureAgents data.

2. **Claude Code fallback mode** — If the SureAgents archive is absent or
...
3. **Neither available** — Ask the user for their SureAgents plans directory or
...
**SureAgents mode:** Verify the plans directory contains `*-denied.md` files. If
...
```
ls ${SUREAGENTS_DATA_DIR:-~/.sureagents}/plans/compound-planning-report*.html
```
...
**Claude Code fallback mode:** The SureAgents inventory fields above do not apply.
...
instead of SureAgents markdown files.
...
- **Footer:** After section 7. Top border, centered italic Playfair Display tagline
  summarizing the corpus (e.g., "Analysis of X denied plans from the SureAgents
  archive.").
...
- **Claude Code fallback mode:** Explicitly label the report source as Claude Code
  `ExitPlanMode` denial reasons. Do not fabricate SureAgents-only fields such as
...
instead of SureAgents file counts.
...
and writes them to a file that SureAgents's `EnterPlanMode` hook can inject into
...
```
${SUREAGENTS_DATA_DIR:-~/.sureagents}/hooks/compound/enterplanmode-improve-hook.txt
```
...
- **Data source priority:** SureAgents is the first-class path. Claude Code log
  analysis is the secondary path for users without SureAgents archives.
