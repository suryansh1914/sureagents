---
name: sureagents-setup-goal
disable-model-invocation: true
description: Turn an idea or objective into a goal package for /goal. Interviews the user, builds a reviewed fact sheet via SureAgents, then explores the codebase to produce an execution plan.
---
...
Build a compact bundle of questions that can derive every "fact" this goal should produce. Package the questions together so the user can answer them quickly in the SureAgents goal setup UI. For each question, include your recommended answer and use options when they make answering faster.
...
```bash
sureagents setup-goal interview goals/<slug>/interview.json --json
```
...
```bash
sureagents setup-goal interview goals/<slug>/interview.json --json | tee goals/<slug>/interview-result.json
```
...
```bash
sureagents setup-goal facts goals/<slug>/facts-review.json --json
```
...
```bash
sureagents setup-goal facts goals/<slug>/facts-review.json --json | tee goals/<slug>/facts-result.json
```
...
Gate the plan with SureAgents:

```bash
sureagents annotate goals/<slug>/plan.md --gate
```
