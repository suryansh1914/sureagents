---
name: sureagents-visual-explainer
disable-model-invocation: true
description: >
  Generate self-contained HTML visualizations with SureAgents theming. Use for implementation
  plans, PR explainers, architecture diagrams, data tables, slide decks, and any visual
  explanation of technical concepts. Plans and PR explainers follow SureAgents's prescriptive
...
**Everything else** (architecture diagrams, data tables, slide decks, project recaps, general visual explanations) → Follow the [Visual explainer path](#visual-explainer-path). Delegates to nicobailon/visual-explainer with SureAgents theme tokens.

## Delivery

Always deliver via SureAgents's annotation UI. Do NOT use `open` or `xdg-open`.

**Plans/proposals** (user should approve/deny):
```bash
sureagents annotate <file> --render-html --gate
```

**Everything else** (informational):
```bash
sureagents annotate <file> --render-html
```
...
1. `references/design-system.md` — SureAgents theme tokens, typography, component patterns
...
4. Read `references/theme-override.md` — SureAgents tokens replacing Nico's palettes

Follow visual-explainer's structure, component classes (`.ve-card`, `.kpi-card`, `.pipeline`), and anti-slop rules. The only override is the color/typography layer — SureAgents tokens instead of Nico's custom palettes.
