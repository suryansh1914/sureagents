...
// ---------------------------------------------------------------------------
// Debug log — only active when SUREAGENTS_DEBUG is set
// ---------------------------------------------------------------------------

const DATA_DIR = getSureAgentsDataDir();
const DEBUG_ENABLED = !!process.env.SUREAGENTS_DEBUG;
...
export function generateOutputPath(): string {
  return join(tmpdir(), `sureagents-codex-${crypto.randomUUID()}.json`);
}
