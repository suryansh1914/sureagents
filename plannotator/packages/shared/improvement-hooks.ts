 ...
 *
 * Reads improvement hook files from ~/.sureagents/hooks/.
 * Falls back to the legacy path (~/.sureagents/) when the new-path
 ...
 * - Same trust model as ~/.sureagents/config.json
 */
...
const DATA_DIR = getSureAgentsDataDir();
 ...
 * `path` is relative to HOOKS_BASE_DIR (~/.sureagents/hooks/).
 * `legacyPath` is relative to LEGACY_BASE_DIR (~/.sureagents/).
 */
const KNOWN_HOOKS = {
