/**
 * SureAgents Flavored Markdown reminder.
 *
 * Static prose injected into the EnterPlanMode PreToolUse hook when the user
 * has opted in via `pfmReminder: true` in ~/.sureagents/config.json. It tells
 * the planning agent which markdown extensions SureAgents's viewer renders so
...
export const PFM_REMINDER = `[SureAgents Flavored Markdown]
This plan will be reviewed in SureAgents, which renders GitHub Flavored Markdown plus the extensions below. Use these features when they make the plan clearer for the reviewer — don't force them in for their own sake.

Code-file links (highest leverage)
Reference real source files inline. SureAgents validates the path and renders a clickable badge that opens the file in the reviewer's editor — prefer this over pasting code when you just need to point at something.
