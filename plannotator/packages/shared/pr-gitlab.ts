...
export async function submitGlMRReview(
  ...
  if (fileComments.length > 0) {
    ...
    if (errors.length > 0) {
      ...
      try {
        const dir = join(getSureAgentsDataDir(), "failed-comments");
        ...
        console.error(`[sureagents] Failed to persist unposted comments: ${writeErr instanceof Error ? writeErr.message : String(writeErr)}`);
      }
      ...
      console.error(
        `[sureagents] ${errors.length}/${fileComments.length} inline comments failed${suffix}:\n${errors.join("\n")}`,
      );
    }
  }
