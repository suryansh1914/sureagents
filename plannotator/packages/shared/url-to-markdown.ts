  ...
  if (options.useJina && !local) {
    try {
      ...
      process.stderr.write(
        `[sureagents] Warning: Jina Reader failed (${err instanceof Error ? err.message : String(err)}), falling back to direct fetch...\n`,
      );
    }
  }
...
async function fetchRawText(url: string): Promise<string | null> {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 (compatible; SureAgents/1.0; +https://sureagents.ai)" };
  try {
    let currentUrl = url;
...
async function fetchViaContentNegotiation(url: string): Promise<string | null> {
  ...
  const headers = {
    "User-Agent": "Mozilla/5.0 (compatible; SureAgents/1.0; +https://sureagents.ai)",
...
async function fetchViaTurndown(url: string): Promise<string> {
  ...
  const headers = {
    "User-Agent":
      "Mozilla/5.0 (compatible; SureAgents/1.0; +https://sureagents.ai)",
