...
describe("getCallbackConfig", () => {
  test("returns null when no cb/ct params", () => {
    expect(getCallbackConfig(loc("https://share.sureagents.ai/#abc123"))).toBeNull();
  });

  test("returns config with params before #", () => {
    const result = getCallbackConfig(
      loc("https://share.sureagents.ai/?cb=https%3A%2F%2Flocalhost%3A9456%2Fsureagents-cb&ct=tok-123#abc"),
    );
    expect(result).not.toBeNull();
    expect(result!.callbackUrl).toBe("https://localhost:9456/sureagents-cb");
    expect(result!.token).toBe("tok-123");
  });

  test("returns config with params after # fragment", () => {
    const result = getCallbackConfig(
      loc("https://share.sureagents.ai/#abc?cb=https%3A%2F%2Flocalhost%3A9456%2Fsureagents-cb&ct=tok-456"),
    );
    expect(result).not.toBeNull();
    expect(result!.callbackUrl).toBe("https://localhost:9456/sureagents-cb");
  ...
  test("returns null when only cb is present", () => {
    expect(
      getCallbackConfig(loc("https://share.sureagents.ai/?cb=https%3A%2F%2Flocalhost%3A9456%2Fcb")),
    ).toBeNull();
  });

  test("returns null when only ct is present", () => {
    expect(getCallbackConfig(loc("https://share.sureagents.ai/?ct=tok-789"))).toBeNull();
  });

  test("decodes encoded callback URL", () => {
    const encoded = encodeURIComponent("https://bot.internal/sureagents-cb");
    const result = getCallbackConfig(loc(`https://share.sureagents.ai/?cb=${encoded}&ct=tok-abc#hash`));
    expect(result!.callbackUrl).toBe("https://bot.internal/sureagents-cb");
  });

  test("returns null when params are empty strings", () => {
    expect(getCallbackConfig(loc("https://share.sureagents.ai/?cb=&ct="))).toBeNull();
  });

  test("partial params in hash — only cb, no ct", () => {
    expect(
      getCallbackConfig(loc("https://share.sureagents.ai/#abc?cb=https%3A%2F%2Flocalhost%3A9456%2Fcb")),
    ).toBeNull();
  });

  test("K8s cluster URL (http) is accepted", () => {
    const k8sUrl = "http://sureagents-cb.svc.cluster.local:9456/callback";
    const result = getCallbackConfig(
      loc(`https://share.sureagents.ai/?cb=${encodeURIComponent(k8sUrl)}&ct=k8s-tok-xyz`),
    );
  ...
  test("rejects non-http/https schemes", () => {
    expect(getCallbackConfig(loc(`https://share.sureagents.ai/?cb=${encodeURIComponent("file:///etc/passwd")}&ct=tok`))).toBeNull();
    expect(getCallbackConfig(loc(`https://share.sureagents.ai/?cb=${encodeURIComponent("javascript:alert(1)")}&ct=tok`))).toBeNull();
  });
  ...
  test("preserves percent-encoded chars in callback URL query params", () => {
    ...
    const result = getCallbackConfig(
      loc(`https://share.sureagents.ai/?cb=${encodeURIComponent(presignedUrl)}&ct=tok-presigned`),
    );
    expect(result).not.toBeNull();
  ...
  test("rejects malformed URL", () => {
    expect(getCallbackConfig(loc(`https://share.sureagents.ai/?cb=not-a-url&ct=tok`))).toBeNull();
  });
});

// --- executeCallback ---

const mockConfig = { callbackUrl: "https://localhost:9456/sureagents-cb", token: "tok-test" };
const mockAnnotatedUrl = "https://share.sureagents.ai/#abc123";
