#!/bin/bash
# Sandbox script for testing SureAgents OpenCode plugin locally
#
# Usage:
...
#   4. Creates sample files with uncommitted changes (for /sureagents-review)
...
#   - Code review: Run /sureagents-review to review the sample changes
...
PLUGIN_LOADER_RELATIVE_PATH="./.opencode/sureagents.ts"
...
if [ "$ISOLATED" = true ]; then
  if [ -z "$ISOLATION_ROOT" ]; then
    ISOLATION_ROOT=$(mktemp -d /tmp/sureagents-opencode-isolated-XXXXXX)
  else
  ...
  export SUREAGENTS_BIN="${SUREAGENTS_BIN:-$PROJECT_ROOT/bin/sureagents.js}"
...
write_runtime_helpers() {
  ...
  cat > "$SANDBOX_DIR/sureagents-opencode-env.sh" << EOF
# Source this file to reuse the isolated OpenCode/SureAgents sandbox.
export SUREAGENTS_OPENCODE_SANDBOX=$(shell_quote "$SANDBOX_DIR")
export SUREAGENTS_OPENCODE_ISOLATION_ROOT=$(shell_quote "$ISOLATION_ROOT")
...
export SUREAGENTS_BIN=$(shell_quote "$SUREAGENTS_BIN")
...
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
. "$SCRIPT_DIR/sureagents-opencode-env.sh"
cd "$SUREAGENTS_OPENCODE_SANDBOX"
...
echo "=== SureAgents OpenCode Sandbox ==="
echo ""
...
echo ""
if [ "$NO_GIT" = false ]; then
  echo "Git status (uncommitted changes for /sureagents-review):"
  git diff --stat
else
...
cat > .opencode/sureagents.ts << EOF
// Loader for local SureAgents plugin development
...
case "$WORKFLOW" in
  manual)
    ...
    echo "  2. Manual review: run /sureagents-last or /sureagents-annotate"
    ;;
  plan-agent)
...
if [ "$NO_GIT" = false ]; then
  echo "  3. Code review: Run /sureagents-review"
fi
echo "  4. Folder draft repro:"
echo "     /sureagents-annotate docs/folder-draft-a"
echo "     Type a draft in the browser, wait a few seconds, then close the tab without sending feedback"
echo "     /sureagents-annotate docs/folder-draft-b"
