...
This guide helps you test UI changes in SureAgents. Whether you're adding new features or fixing bugs, follow these
...
- **`apps/`** - Deployable applications
  ...
  - `portal/` - Share portal (share.sureagents.ai)
  - `marketing/` - Marketing site (sureagents.ai)
...
# Portal (share.sureagents.ai)
bun run dev:portal

# Marketing site (sureagents.ai)
...
4. Enables `hooks` and registers a `Stop` hook pointing at the local SureAgents entrypoint
5. Runs a real `codex exec` prompt that returns only a `<proposed_plan>` block
6. Leaves behind rollout logs, SureAgents history, plan files, and session URLs in an artifact directory

This is the best harness when you want to verify the full Codex deny/revise/approve loop instead of simulating hook
payloads. For browser automation, set `SUREAGENTS_BROWSER=/usr/bin/true`, keep the script running in one terminal,
...
```
Error: Cannot find module '@sureagents/ui'
```
